...
| Taskbar/tray settings and wardrobe UX surface | No tray menu, no settings UI, no wardrobe UI pipeline currently wired. | `Adapt` | Desktop shell controls are a required roadmap surface. | `02-architecture-capability-registry`, `07c-shell-settings-and-wardrobe-surface`, `08-test-and-acceptance-matrix` | D02 ownership boundaries + D07c shell contracts + D08 shell acceptance tests. |
...
| Proactive pet-to-user communication triggers/channels | No proactive message trigger pipeline in current app. | `Adapt` | ADR-0008 requires bounded, logged, rate-limited announcements. | `03-pet-core-events-intents-suggestions`, `04-openclaw-bridge-spec`, `07b-dialog-surface-and-minimal-offline-loop`, `08-test-and-acceptance-matrix` | D03 adds `PET_ANNOUNCEMENT`; D04 defines local gating before output; D07b makes the output visible. |
...
| Conversation pathways (text + voice with offline fallback) | No chat UI, no STT/TTS capability contracts. | `Adapt` | ADR-0007 requires text-first resilient operation and optional voice add-ons. | `03-pet-core-events-intents-suggestions`, `04-openclaw-bridge-spec`, `07b-dialog-surface-and-minimal-offline-loop`, `08-test-and-acceptance-matrix` | D03 event schemas + D04 fallback sequences + D07b visible dialog surface + D08 online/offline checks. |
| Speech bubble/dialog fallback + lip-sync approximation | Visual rig exists; no speech-driven bubble system or lip-sync contract exists. | `Adapt` | Required conversation feedback layer and degraded-mode behavior. | `07b-dialog-surface-and-minimal-offline-loop`, `04-openclaw-bridge-spec`, `08-test-and-acceptance-matrix` | D07b dialog/bubble/talk feedback bindings; D08 verifies fallback talk mode when TTS unavailable. |
